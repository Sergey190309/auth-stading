...
async def test_create_user_if_not_exists(
    ...
    async_engine_factory_func: Callable[[str], AsyncIterator[AsyncEngine]],
    async_admin_url: str,
) -> None:
    """Clean up if exists"""
    async for engine in async_engine_factory_func(async_admin_url):
...
async def test_grant_all_preveleges(
    ...
    async for engine in async_engine_factory_func(async_admin_url_test_db):
        async with engine.connect() as conn:
            messages = await grant_all_preveleges(
                db_name=test_db_name,
                user_name=test_user_name,
